...
public class RenumIncrementFilesHandler extends AbstractHandler implements IStartup {
    @Override
...
//            try {
//				duplicateFile(files.getFirst());
//			} catch (CoreException e) {
//				// TODO Auto-generated catch block
//				e.printStackTrace();
//			}
            List<IFile> newFiles = renumberFiles(files, true);
    ...
    private IFile duplicateFile(IFile originalFile) throws CoreException {
        // Use the fixed new file name "V002_empty.sql"
        String newFileName = extractNumber(originalFile) + "_empty.sql";

        // Create the new file path
        IPath newFilePath = originalFile.getParent().getFullPath().append(newFileName);

        // Check if a file with the new name already exists
        IFile newFile = originalFile.getParent().getFile(newFilePath);
        if (newFile.exists()) {
            System.out.println("Target file already exists: " + newFileName);
            return newFile; // Return the existing file or handle the case as needed
        }

        // Create a new file with the fixed name "V002_empty.sql"
        newFile.create(originalFile.getContents(), true, new NullProgressMonitor());
